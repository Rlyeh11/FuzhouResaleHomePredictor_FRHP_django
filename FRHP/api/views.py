...
RF= joblib.load(f'api/model/RF/RF_{model_date}.model')
    ...
    if request.method == 'GET':
        ...
        if features is not None:
            try:
                ...
                print(clusters)
                GBRT_pred = [GBRT[cluster].predict(module_features_without_position.iloc[i].values.reshape(1, -1))[0]
                             for i, cluster in enumerate(clusters)]
                RF_pred = [RF[cluster].predict(module_features_without_position.iloc[i].values.reshape(1, -1))[0]
                           for i, cluster in enumerate(clusters)]
                print('::features::')
            ...
            return JsonResponse({'status': '200', 'msg': 'request post successfully', 'data': {'GBRT':round(GBRT_pred[0], 2), 'RF':round(RF_pred[0], 2)}})
        else:
